...
<Aside type="note">
  Even though the cache is shared, each `createApiClient` owns its **own
  `TagRegistry`**. So `internal.useM("createPost", ...)` will not invalidate
  `public_.useQ("listPosts", ...)`, which is what you want — tag namespaces
