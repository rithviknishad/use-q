...
`use-q` is written in strict TypeScript with `exactOptionalPropertyTypes` and `noUncheckedIndexedAccess`. For the best inference, enable strict mode in your `tsconfig.json`:
