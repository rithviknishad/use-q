...
See [Codegen](/docs/core/codegen) for CLI flags.
...
If your monorepo has more than one upstream API, give each its own schema package — `@my-org/internal-schema`, `@my-org/billing-schema`, etc. Then build a `createApiClient` per API in the consumer, sharing one `QueryClient`. See [BYO QueryClient](/docs/guides/byo-query-client).
